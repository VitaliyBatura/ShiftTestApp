...
import org.example.sort.SortMode;
...
public class CmdParser {
    ...
    public CmdParser(String[] args) throws IllegalArgumentException {
        if (args.length < 3) {
            throw new IllegalArgumentException("Error: Invalid number of arguments. Required: " +
                    "data type, output file name, input file names (at least one)");
        }
        for (String arg : args) {
            switch (arg) {
                case "-a":
        ...
        if (dataType == null) {
            throw new IllegalArgumentException("Error: Data type is required (use -s for String or -i for Integer)");
        }
        if (outputFile == null) {
